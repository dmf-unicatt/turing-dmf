...
"""Test mathrace_interaction.filter.reorder_lists_in_imported_turing."""
...
def random_dict(turing_dict: mathrace_interaction.typing.TuringDict) -> mathrace_interaction.typing.TuringDict:
    """Randomly reorder lists in the provided dictionary."""
    turing_dict_copy = dict(turing_dict)
    for key in ("eventi", "soluzioni", "squadre"):
        key_len = len(turing_dict[key])
        turing_dict_copy[key] = [turing_dict[key][r] for r in random.sample(range(key_len), k=key_len)]
    return turing_dict_copy


def test_reorder_lists_in_imported_turing(
    random_dict: mathrace_interaction.typing.TuringDict, turing_dict: mathrace_interaction.typing.TuringDict
) -> None:
    """Test reorder_lists_in_imported_turing."""
    mathrace_interaction.filter.reorder_lists_in_imported_turing(random_dict)
    assert random_dict == turing_dict


@pytest.mark.parametrize("input_file_option,output_file_option", [("-i", "-o"), ("--input-file", "--output-file")])
def test_reorder_lists_in_imported_turing_entrypoint(
    random_dict: mathrace_interaction.typing.TuringDict, turing_dict: mathrace_interaction.typing.TuringDict,
    run_entrypoint: mathrace_interaction.typing.RunEntrypointFixtureType,
    input_file_option: str, output_file_option: str
) -> None:
    """Test running reorder_lists_in_imported_turing as entrypoint."""
    with tempfile.NamedTemporaryFile() as input_json_file, tempfile.NamedTemporaryFile() as output_json_file:
        ...
        stdout, stderr = run_entrypoint(
            "mathrace_interaction.filter.reorder_lists_in_imported_turing", [
