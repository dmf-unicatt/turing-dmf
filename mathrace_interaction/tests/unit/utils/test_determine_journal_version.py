...
def test_determine_journal_version_entrypoint(
    journal: io.StringIO, journal_version: str, run_entrypoint: RunEntrypointFixtureType, input_file_option: str
) -> None:
...
def test_determine_journal_version_error_on_empty_file(
    ...
    runtime_error_contains(lambda: determine_journal_version(wrong_journal), "The provided journal is empty")
...
0 222 inizio gara
""")
    runtime_error_contains(lambda: determine_journal_version(wrong_journal), "No race start event detected")
