...
"""Test mathrace_interaction.utils.journal_event_filterer_by_id."""
...
from mathrace_interaction.utils.journal_event_filterer_by_id import journal_event_filterer_by_id
from mathrace_interaction.utils.journal_event_filterer_by_timestamp import journal_event_filterer_by_timestamp
...
def test_journal_event_filterer_by_id(journal: io.StringIO) -> None:
    ...
    filtered_by_id = journal_event_filterer_by_id(journal, 7)
    # Obtain an equivalent journal using a timestamp based filter at the same time when event 7 happens
    journal.seek(0)
    filtered_by_timestamp = journal_event_filterer_by_timestamp(journal, "450")
...
def test_journal_event_filterer_by_id_entrypoint(
    journal: io.StringIO, run_entrypoint: RunEntrypointFixtureType, input_file_option: str,
    id_upper_bound_option: str, output_file_option: str
) -> None:
