...
if [[ -f "${CONTAINER_ID_FILE}" ]]; then
    ...
    CONTAINER_ID=$(docker create ${NETWORK_PROPERTIES} -v /tmp/shared-turing-dmf:/shared/host-tmp -v $(dirname ${PWD}):/shared/git-repo -v ${VOLUME_ID}:/mnt -e DOCKERHOSTNAME=$(cat /etc/hostname) -e TZ=$(cat /etc/timezone) -e TERM=${TERM} ghcr.io/dmf-unicatt/turing-dmf:latest)
