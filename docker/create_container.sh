...
if [[ -f "${CONTAINER_ID_FILE}" ]]; then
    ...
    CONTAINER_ID=$(docker create --net ${NETWORK_ID} --ip 10.200.1.23 --mac-address 02:42:0a:c8:01:17 -p 80:80 -v /tmp/shared:/tmp/shared -v ${VOLUME_ID}:/mnt -e DOCKERHOSTNAME=$(cat /etc/hostname) turing-dmf:latest)
